...
export const updateFilialContacts = async (filialText, phone, whatsappPhone, whatsappLink, instagram, telegramId, telegramLink) => {
    try {
        const response = await axios.post(`${configUrl}/api/filial/updateFilialContacts`, {filialText, phone, whatsappPhone, whatsappLink, instagram, telegramId, telegramLink});
        return response.data;
    } catch (error) {
        if (error.response) {
...
export const getFilialContacts = async (filialText) => {
    try {
        const response = await axios.get(`${configUrl}/api/filial/getFilialContacts/${filialText}`);
        return response.data;
    } catch (error) {
...
export const updateFilialSettings = async (filialText, videoLink, chinaAddress, whatsappNumber, aboutUsText, prohibitedItemsText, contractFile) => {
    const formData = new FormData();
    formData.append('filialText', filialText);
    if (videoLink) formData.append('videoLink', videoLink);
    if (chinaAddress) formData.append('chinaAddress', chinaAddress);
    if (whatsappNumber) formData.append('whatsappNumber', whatsappNumber);
    if (aboutUsText) formData.append('aboutUsText', aboutUsText);
    ...
    try {
        const response = await axios.post(`${configUrl}/api/filial/updateFilialSettings`, formData, {
            headers: {
...
export const getFilialSettings = async (filialId) => {
    try {
        const response = await axios.get(`${configUrl}/api/filial/getFilialSettings/${filialId}`);
        return response.data;
    } catch (error) {
